...
const returnNames = (arr) => arr.reduce((newArr, character, idx) => { 
  newArr[idx] = character.name;
...
const reversedString = (arr) => arr.reduce((newString, letter, idx) => {
  letter += newString;
...
const countNumberOfChildren = (arr) => arr.reduce((totalChildren, character, idx) => {
  (character.children) ? totalChildren += character.children.length : totalChildren + 0;
...
const calculateAverage = (arr) => arr.reduce((sum, count) => {
  sum += count;
