...
const evenOdd = (arr) => {
  arr.map((num) => (typeof(num) !== 'number' ? num = 'N/A' : (num % 2 === 0) ? num = 'even' : num = 'odd'));
};
...
const extractStats = (arr) => arr.map((stat) => `Object { "name": ${ stat.name }, "total": ${ stat.effort + stat.baseStat } }`);
